...
const BookCard = ({ book, onDelete }) => {
  ...
  return (
  <div
    className="group relative rounded-xl overflow-hidden border border-gray-100 hover:border-gray-200 transition-all duration-300 hover:shadow-xl hover:shadow-gray-100/50 cursor-pointer hover:-translate-y-1"
  ...
  >
    ...
    <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex gap-2">
      ...
      <button className="w-8 h-8 bg-white/90 backdrop-blur-sm rounded-full flex items-center justify-center shadow-lg hover:bg-red/50 transition-colors group/delete cursor-pointer"
        onClick={(e) => {
          e.stopPropagation();
          onDelete(book._id);
        }}
